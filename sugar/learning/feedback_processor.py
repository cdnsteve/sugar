...
class FeedbackProcessor:
    ...
    async def process_feedback(self) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_tasks = await self.work_queue.get_recent_work(limit=50, status='completed')
            failed_tasks = await self.work_queue.get_recent_work(limit=20, status='failed')
            
            # Generate comprehensive insights
            insights = {
                'success_patterns': await self._analyze_success_patterns(completed_tasks),
                'failure_patterns': await self._analyze_failure_patterns(failed_tasks),
                'performance_metrics': await self._calculate_performance_metrics(completed_tasks, failed_tasks),
                'priority_effectiveness': await self._analyze_priority_effectiveness(completed_tasks),
                'discovery_source_effectiveness': await self._analyze_discovery_effectiveness(completed_tasks),
                'execution_time_patterns': await self._analyze_execution_times(completed_tasks),
                'recommendations': await self._generate_recommendations(completed_tasks, failed_tasks),
            ...
            logger.info(f"🧠 Processed feedback from {len(completed_tasks)} completed and {len(failed_tasks)} failed tasks")
            return insights
    ...
    async def _analyze_success_patterns(self, completed_tasks: List[Dict]) -> Dict[str, Any]:
        ...
        for task in completed_tasks:
            ...
            if task.get('result'):
                success_indicators = await self._extract_success_indicators(task['result'])
    ...
    async def _analyze_failure_patterns(self, failed_tasks: List[Dict]) -> Dict[str, Any]:
    ...
    async def _calculate_performance_metrics(self, completed_tasks: List[Dict], 
                                           failed_tasks: List[Dict]) -> Dict[str, Any]:
        ...
        if completed_tasks:
            ...
            dates = [task['completed_at'] for task in completed_tasks if task.get('completed_at')]
            if len(dates) > 1:
                dates.sort()
                time_span = (datetime.fromisoformat(dates[-1].replace('Z', '')) - 
        ...
        return {
            ...
            'average_attempts_per_task': statistics.mean([task['attempts'] for task in completed_tasks + failed_tasks]) if total_tasks > 0 else 0
        }
    
    async def _analyze_priority_effectiveness(self, completed_tasks: List[Dict]) -> Dict[str, Any]:
        ...
        for priority, tasks in priority_analysis.items():
            if tasks:
                ...
                effectiveness[priority] = {
                    ...
                    'efficiency_score': len(tasks) / (avg_time * avg_attempts)  # More tasks, less time/attempts = better
                }
        
        return effectiveness
    
    async def _analyze_discovery_effectiveness(self, completed_tasks: List[Dict]) -> Dict[str, Any]:
    ...
    async def _analyze_execution_times(self, completed_tasks: List[Dict]) -> Dict[str, Any]:
        ...
        for task in completed_tasks:
            ...
            if exec_time:
                ...
                time_patterns['by_source'][task.get('source', 'unknown')].append(exec_time)
    ...
    async def _generate_recommendations(self, completed_tasks: List[Dict], 
                                      failed_tasks: List[Dict]) -> List[Dict[str, Any]]:
        """Generate actionable recommendations based on learning"""
        recommendations = []
        
        total_tasks = len(completed_tasks) + len(failed_tasks)
        if total_tasks < 5:  # Need sufficient data
            return [{'type': 'info', 'message': 'Collecting data - recommendations will improve with more task history'}]
        ...
        for rec in recommendations:
            ...
            if rec_type == 'priority_adjustment':
                ...
                if 'error_monitor' in action:
                    adaptations['discovery_adjustments']['boost_error_monitoring'] = True
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_processing_time": self.learning_cache.get('last_insights', {}).get('timestamp'),
