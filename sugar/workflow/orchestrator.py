...
class WorkflowOrchestrator:
    ...
    def _load_workflow_config(self) -> Dict[str, Any]:
        ...
        if profile == WorkflowProfile.SOLO:
            defaults = {
                ...
                'discovery': {
                    'handle_internally': True  # No external issue creation
                }
            }
    ...
    def get_workflow_for_work_item(self, work_item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if source_type == 'github_watcher':
            ...
            github_config = self.config.get('sugar', {}).get('discovery', {}).get('github', {})
            git_workflow = github_config.get('workflow', {}).get('git_workflow', 'direct_commit')
        ...
        logger.debug(f"🔄 Determined workflow for {source_type}/{work_type}: {workflow['git_workflow'].value}")
        return workflow
    ...
    def format_commit_message(self, work_item: Dict[str, Any], workflow: Dict[str, Any]) -> str:
        ...
        if workflow['git_workflow'] == WorkflowType.PULL_REQUEST and self.git_ops:
            ...
            try:
                ...
                if success:
                    ...
                    logger.warning(f"⚠️ Failed to create branch {branch_name}, using current branch")
                    workflow['git_workflow'] = WorkflowType.DIRECT_COMMIT
            except Exception as e:
                logger.warning(f"⚠️ Branch creation failed, falling back to direct commit: {e}")
                workflow['git_workflow'] = WorkflowType.DIRECT_COMMIT
        
        return workflow
        
    async def complete_work_execution(self, work_item: Dict[str, Any], workflow: Dict[str, Any], 
